...
def get_vif_stats():
    ...
    for style in sorted(list(set(vifs.columns) - {"date"})):
        vif_dict[style] = np.round(
            sum(abs(vifs[style].astype(float)) > 5) / len(vifs), 3
        )
    return {
        "len": len(vifs),
        "last_date": last_date,
...
def get_tstats_stats():
    ...
    stats = (
        TStatistics.objects.using("factor_model_estimates")
        .values(*field_names)
    )
    ...
    for style in sorted(list(set(stats.columns) - {"date"})):
        tstat_dict[style] = np.round(sum(abs(stats[style]) > 1.96) / len(stats), 3)
    return {
        "len": len(stats),
        "last_date": last_date,
        "first_date": first_date,
        "active_tstat_ration": tstat_dict,
    }
...
def get_rsquares():
    ...
    rsquares = (
        RSquares.objects.using("factor_model_estimates")
        .values(*field_names)
    )
    rsquares = pd.DataFrame(list(rsquares))
    rsquare_dict = {}
    rsquare_dict["len"] = len(rsquares)
    rsquare_dict["avg_core_r2"] = np.nanmean(rsquares["r2_core"])
    rsquare_dict["avg_core_r2_adj"] = np.nanmean(rsquares["r2_adj"])
