...
def calculate_lognormal_es_var(portfolio_std: float, confidence_level: float) -> tuple[float, float]:
    """
    ...
    es = 1 - np.exp(0.5 * portfolio_std**2) * norm.cdf(
        norm.ppf(1 - confidence_level) - portfolio_std
    ) / (1 - confidence_level)
